...
//		}
	...
	public boolean isDatPresentOnDashboard(String sNumeroSS) {
		searchSNumeroSS(sNumeroSS);
		return waitForSNumeroSSEle(sNumeroSS);
	 ...
	 * @param sNumeroSS
	 */
	public void searchSNumeroSS(String sNumeroSS) {
		waitForElementToBeVisible(TXT_FIELD_SEARCH_DAT);
		sendKeyToElement(TXT_FIELD_SEARCH_DAT, sNumeroSS);
	}
	
	/**
	 * Wait for sNumeroSSEle display
	 * @param sNumeroSS
	 * @return boolean
	 */
...
//		clickDatActionButton();
	...
	public void importDATF(String sFileName, String succVerificationXPath) throws AWTException {

		LOGGER.info("Importing dat from " + sFileName);
		//importDATButton.selectFileByCopyPaste(sFileName);
		//importDATButton.selectFile(getResourcePath("") + File.separator + sFileName);
		//importDATButton.typeKeys(sFileName);
		LOGGER.info("Sending keys to upload-file element: " + sFileName);
		
		browserFindElement(By.xpath(IMPORT_DAT)).sendKeys(sFileName);
//		loadFileByRobot(sFileName);

//		Object fileImport = executor.executeScript("return document.getElementById('import-dat')");
//		((WebElement) fileImport).sendKeys(sFileName);
...
//		clickEmalOutlineIcon.click();
		waitForElementVisibilityOfElementLocated(BTN_ENVOYER_POUR_REVISION);
//		new  CustomWebDriverWait(browser, 3).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(BTN_ENVOYER_POUR_REVISION)));
//		envoyerPourRevision.click();
		clickHiddenElementUsingJavaScript(BTN_ENVOYER_POUR_REVISION);
		//new CustomWebDriverWait(browser, 5).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(APPOUVER_LA_DAT_SUCCESS_MESSAGE)));

		//close the success popup
//		closeComment.click();
//		new CustomWebDriverWait(browser, 5).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(BTN_LOCK)));
//		lockButton.shouldBeDisplayed();
...
//		//click on approuver la DAT icon
//		WebElement approuverLaDat = new CustomWebDriverWait(browser, 5).until(ExpectedConditions.presenceOfElementLocated(By.xpath(BTN_APPROUVER_LA_DAT)));
//		executor.executeScript("arguments[0].click();", approuverLaDat);
//		//approuverLaDAT.click();
...
//		WebElement approuverLaDatConfirm = new CustomWebDriverWait(browser, 5).until(ExpectedConditions.presenceOfElementLocated(By.xpath(BTN_APPROUVER_LA_DAT_CONFIRM)));
//		executor.executeScript("arguments[0].click();", approuverLaDatConfirm);
//		//approuverLaDAT.click();
//		LOGGER.info("Clicked on "+By.xpath(BTN_APPROUVER_LA_DAT_CONFIRM));
		//new CustomWebDriverWait(browser, 5).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(BTN_APPROUVER_LA_DAT_CONFIRM)));

		// click on confirm button
		//approuverLaDATConfirm.click();
//		new CustomWebDriverWait(browser, 3).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(APPOUVER_LA_DAT_SUCCESS_MESSAGE)));
...
//			approuveeParAvocate.shouldBeDisplayed();
...
//			new CustomWebDriverWait(browser, 4).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(ENVOI_SUCCESS_MESSAGE)));
