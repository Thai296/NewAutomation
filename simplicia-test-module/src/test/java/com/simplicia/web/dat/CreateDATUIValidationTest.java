...
public class CreateDATUIValidationTest extends SeleniumTestAsSimpliciaUser {
    ...
    public void dat_Test_04_Import() throws Exception {
        ...
        try {
            ...
            LOGGER.info("*** Step 2 Action: Navigate to Mes DATs and click on Importer une DAT to import the recently downloaded DAT***");
            navigateToDatOption("Mes DATs", true);
            // import the file
    ...
    public void dat_Test_05_Lawyer_Review() throws Exception {
        ...
        try {
            ...
            // click on chat icon , send message to lawyer, close the comment dialog
            //String sComment = datMainPage.echangerAvecAvocat("Sending the DAT for review");
            ...
            if(bSent) {
                println("Dat was sent to lawyer successfully: " + sImportedDATNumeroSS);

                //logout and login using lawyer credentials

                println("Logging out, and then login as lawyer");
                simpliciaReusableActions.logOut();
                simpliciaReusableActions.logIn(TestData.LAWYEREMAIL, TestData.PASSWORD);
                Thread.sleep(15000);
                println("Logged in as lawyer " + TestData.LAWYEREMAIL);
                // Navigate to left navigation menu
                navigateToDatOption("Mes DATs", true);
//                sleepSilently(10000);
                println("Lawyer checking if dat is found: " + sImportedDATNumeroSS);
                bDat = datMainPage.isDatPresentOnDashboard(sImportedDATNumeroSS);
                if(bDat) {
                    println("Lawyer checked, dat is found: " + sImportedDATNumeroSS);
                    ...
                    println("Lawyer approved dat: " + sImportedDATNumeroSS);
                    ...
                    if(bApproved){
                        ...
                        println("Logging out, will log back in as client");
                        simpliciaReusableActions.logOut();

                        // login to user/client
                        simpliciaReusableActions.logIn();
                        Thread.sleep(15000);
                        println("Logged back in as client");
                        navigateToDatOption("Mes DATs", true);
//                        sleepSilently(10000);

                        // validate if approved by lawyer button is displayed or not
                        println("Client checking if dat is found and approved: " + sImportedDATNumeroSS);
                        bDat = datMainPage.isDatPresentOnDashboard(sImportedDATNumeroSS);
                        if (bDat) {
                            println("Dat found: " + sImportedDATNumeroSS);
                        } else {
                            println("Dat NOT found: " + sImportedDATNumeroSS);
                        }
                        if (datMainPage.isDATApproved()) {
                            println("Dat approved: " + sImportedDATNumeroSS);
                        } else {
                            println("Dat NOT approved: " + sImportedDATNumeroSS);
                        }

                        Assert.assertTrue(datMainPage.isDATApproved(), "DAT review is approved by lawyer");
                    } else {
                        println("Lawyer FAILED to approve dat: " + sImportedDATNumeroSS);
                    }
                }
            }
        } catch (Exception e) {
    ...
    public void dat_Test_06_Submit_To_Health_Insurance() throws Exception {
        ...
        try {
            ...
            if (bDat) {
                ...
                println("bIncomplete=" + bIncomplete);
                if(bIncomplete){
                    datMainPage.editDAT();
                    updateDATProfile();
                    saveAndValidateDAT();
    ...
    public void dat_Test_07_ConvertZip_To_XML_CREATE_DAT() throws Exception {
        ...
        try {
          ...
          /*  Path sourcePath = Paths.get(getResourcePath(""));
            ...
            listFilesInDir(getResourcePath(""));
            File source = new File(getResourcePath("") + File.separator + "dat.zip");
...
//    }
    ...
    private String createDAT(boolean bUploadFile, boolean bMyProfile, boolean bPersonneAvisee) throws Exception {
        ...
        println("############################ Victime numero SS is :" + sNumeroSS + " ####################################");
    ...
    private String createDATFromConvertedXML(String sDATDataXMlPath, String sXMLFileName, String fileNameToUpload, boolean bMyProfile, boolean bPersonneAvisee) throws Exception {
        ...
        println("mLAVictimeDetails: " + ToStringBuilder.reflectionToString(mLAVictimeDetails, ToStringStyle.SIMPLE_STYLE));
